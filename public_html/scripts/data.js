  ...
  socket.onmessage = function (event) { 
    ...
    switch (data.event) { //On regarde quel événement est envoyé
        case "dataFromCar":
            ...
            document.getElementById("laps").innerHTML = data.tour;
...
function ouvrir(){
    try{
        ...
        alert("Le fichier out.png n'a pas été trouvé");
    }
}
ouvrir();
img.onload=function(){
...
const supprimercircuit = document.getElementById("supprimercircuit");
...
if(acces==1 || acces==2){
    ...
    supprimercircuit.addEventListener('click',function(){ //On demande la suppression du dossier correspondant au dossier qu'on le supprime
        suppcircform = document.getElementById("suppcircform");
        socket.send(JSON.stringify({
            event:"supprimercircuit",
            dossier:document.getElementById("choixcircuit").options[document.getElementById("choixcircuit").selectedIndex].text
        }));
        setTimeout(function(){
            suppcircform.submit();
        }, 1000);
